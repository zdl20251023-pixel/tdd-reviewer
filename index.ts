...
// Few-Shot 样本：提示词工具产出的高质量审查范例，用于教会深度解析模块那种锐利的洞察力
const FEW_SHOT_PATH = path.resolve(__dirname, "doc", "审查-分析结果.md");
...
const VulnerabilitySchema = z.object({
  level: z
    .enum(["P0", "P1", "P2"])
    .describe("P0=致命架构漏洞 P1=高风险 P2=普通优化点"),
  title: z
    .string()
    .describe(
      "一句话标题，必须引用具体的代码实体/接口名，如：session_manager 的 RPC 复用导致脏数据污染"
    ),
  triggerPath: z
    .string()
    .describe("触发路径：从用户操作到故障发生的完整因果链"),
  underlyingConcept: z
    .string()
    .describe(
      "降维类比：强制映射为 状态机/行为树/ECS/帧同步/状态同步/渲染管线/内存池/AOI 中的一个概念进行剖析"
    ),
...
async function runStateConcurrencySystem(
  entities: Entity[]
...
async function runAiConcurrencySystem(
  entities: Entity[]
): Promise<Vulnerability[]> {
  const targets = entities.filter(
...
async function runChaosInjectionSystem(
  entities: Entity[],
  tddContent: string
): Promise<ChaosResult[]> {
  console.log(
...
async function runAntiCorruptionAuditSystem(
  entities: Entity[]
): Promise<Vulnerability[]> {
...
function renderVulnerabilitiesSection(
...
async function main() {
  console.log(
    "🚀 [System Start] 启动增强型 ECS 审查管线 v3 (Few-Shot Enhanced)..."
  );
  ...
  try {
    ...
    const [tddContent, fewShotExample] = await Promise.all([
      ...
      fs.readFile(FEW_SHOT_PATH, "utf-8").catch(() => ""),
    ]);
...
不要做任何优劣评价，只做客观结构化提取。宁可多提取一个模块，也不要遗漏。`,
    ...
    console.log(
      `✅ [Phase 2] 审查完成：${allVulnerabilities.length} 个漏洞，${p0Vulnerabilities.length} 个 P0，${chaosResults.length} 个混沌测试结果`
    );

    // ==========================================
    // Phase 3: 混合渲染管线 (Few-Shot Enhanced)
    // 结构化数据用模板渲染
    // 评估+优化 和 深度解析 分两路并行 AI 生成
    // 深度解析注入 Few-Shot 样本，教会 AI 那种锐利的洞察力
    // ==========================================
    console.log("🔨 [Phase 3] 启动 Few-Shot 增强混合渲染管线...");

    // 共享的结构化上下文（供两路 AI 调用复用）
    const structuredContext = `【原始 TDD 文档（前4000字）】:
        ...
        ? [
            ...
            },
            {
              ...
              content:
                ...
                fewShotExample, // 如果无法精确匹配第4节，就用全文作为示例
            },
          ]
        : [];

    // 3a. 两路并行 AI 调用
    const [sections13Result, section4Result] = await Promise.all([
      // 路线 A: 方案评估 + 优化建议（不需要 Few-Shot，靠结构化数据驱动）
      generateText({
        model: google("gemini-3-flash-preview"),
        system: `你是一个最终决策节点。像原始人一样回复，只说结果，不废话，不客套。严禁使用任何问候语。
基于传入的原始 TDD 文档、提取的系统实体、P0 漏洞列表和混沌测试结果，输出以下 2 个章节：
...
- [降本增效]：给出具体的 Token 控制或中间件优化策略。`,
      ...
      // 路线 B: 深度解析（注入 Few-Shot 样本，教会 AI 输出锐利的业务洞察）
      generateText({
        model: google("gemini-3-flash-preview"),
...
- [底层逻辑映射]：强制使用上述概念体系进行对比剖析。`,
        messages: [
          // Few-Shot: 先展示一个高质量的"深度解析"输出范例，让 AI 学会那个语气
          ...fewShotMessages,
          // 真正的请求
          {
            role: "user" as const,
...
${structuredContext}`,
    ...
    console.log("✅ [Phase 3] 两路 AI 渲染完成。");

    // 3b. 第 2 节 — 模板硬渲染（100% 格式可控）
    const section2 = `### 2. 潜在风险 (P0 级，严控 ${
      p0Vulnerabilities.length
...
function assembleDocument(
  sections13Text: string,
  section2: string,
  section4Text: string,
  chaosSection: string
): string {
  // 从路线 A 的输出中提取第 1、3 节
  const section1Match = sections13Text.match(
    /### 1\. 方案评估[\s\S]*?(?=### [23]|$)/
  );
  const section3Match = sections13Text.match(/### 3\. 优化建议[\s\S]*?$/);

  const section1 =
    section1Match?.[0]?.trim() ||
    "### 1. 方案评估\n\n- 解析失败，请检查管线输出。";
  const section3 =
    section3Match?.[0]?.trim() ||
    "### 3. 优化建议\n\n- 解析失败，请检查管线输出。";

  // 路线 B 的输出直接就是第 4 节（可能带有 ### 4 前缀，也可能没有）
